...
//
// 这个注释块的头三行曾经是 ConsumeEnrollToken 的，写着「**原子地把 Token
// 标记为已用**」——而那正是这个函数明确**不做**的事。拆开这两个函数的全部
// 意义就在那个区别上，而拆的时候我把新说明加在了旧说明**后面**，
// 没删掉旧的。一个从上往下读的人，第一句话读到的就是反的。
//
// 记在这儿是因为它比「注释过期」更隐蔽：过期的注释描述的是**曾经为真**的行为，
// 而这一句从写下的那一刻起就是错的——它属于另一个函数。
//
