...
type DNSSyncState struct {
	...
	//
	// 旧数据里没有它，那时它是 `null`：**不是「一个目标都没有」**，
	// 是「这条记录写于只支持单域名的版本」。界面据此退回只显示 Detail。
	//
	// **没有 omitempty，这是刻意的。**
	//
	// 带上它的话这个键会整个消失，于是下游要分辨三种写法（不存在 / null /
	// 空数组）而它们只表达两个意思 —— 而 JS 里 `?.` 和 `??` 会把
	// 「不存在」和「null」悄悄合并，那正好抹掉了上面那句区别。
	//
	// 前端拿真主控比形状时撞到的就是这个：契约里写着 null，
	// 而真主控**整个不发这个键**。契约那句没错，是 omitempty 让它没法成立。
	Targets []DNSTargetSync `json:"targets"`
}
