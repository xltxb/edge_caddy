...
func (a *Agent) serve(ctx context.Context, stream tunnelStream, out *tunnelWriter) error {
	...
	for {
		...
		default:
			...
			//
			// 证书续期尤其不该：证书随每次下发内联带上（ADR-0010），
			// 主控续期之后触发一次下发即可，不需要单独的续期指令——
			// 那会做出一条与 ADR-0010 冲突的下发路径。proto 里那条消息
			// 已经删掉并 reserved 4。
			//
			// （这里曾经写着「证书续期在后续工单落地」。那句话是假的，
			// 而且是最坏的一种假：它承诺了一件按现在的架构不该发生的事。
			// 复盘放在结论后面——**一个从上往下读的人，第一句读到什么就先信什么**。）
		}
	}
}
	...
	for {
		...
		case <-t.C:
			...
			if err := out.Send(&edgev1.AgentMsg{
				...
				// **这里刻意不记日志。**
				//
				// a.log 会进缓冲，而缓冲正是送不出去的那个东西：一次失败会生出
				// 一条新日志，下一次再失败再生一条，隧道一直不通就一直长。
				//
				// 注释原先写的是「用 Debug 且不带缓冲的那份」，而下面一条日志
				// 都没有——一句承诺了某个动作、而那个动作不存在的注释（issue #80）。
				// 隧道长期不通时日志上报静默停摆，这是**已知的**代价：
				// 主控那边看得到心跳停了，而心跳与日志走同一条隧道。
				return
			}
		}
	}
