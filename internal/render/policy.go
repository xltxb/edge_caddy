...
func validatePolicies(p Policies) []Issue {
	...
	if p.Log.RateLimit {
		...
		//
		// **而「做不到」这句话现在只对了一半，所以要指路。**
		//
		// 限流做得到了 —— 走的是访问规则里的 rate_limit（Agent 校验端点，
		// ADR-0003）。这个**全局开关**仍然做不到：它要的是 Caddy 自己的
		// 限流模块。两件事不冲突，而人不会这么读——他看到「做不到」就走了，
		// 而他要的东西在隔壁。
		//
		// **一句只说了一半的实话，读起来跟假话一样。**
		issues = append(issues, Issue{"global:log", "spec.rate_limit",
			"这个全局开关做不到：官方 Caddy 没有限流模块（caddy-ratelimit 是插件），" +
				"要用它就得自建 Caddy 二进制，而那会推翻「节点跑官方包」这个前提。" +
				"要限流请在访问控制里新建一条「限流」规则 —— 那条走的是 Agent 的" +
				"校验端点，按域名生效，并且可以按 IP 或 IP+路径计数"})
	}
	return issues
}
