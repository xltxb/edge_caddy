...
**完整的控制台部署步骤见 [控制台部署.md](控制台部署.md)** ——那份按你实际会敲的
顺序写（建库 → 配置 → 初始化 → systemd → 确认起对了 → 加第一台节点）。

下面这几节是那份文档里几个容易出错的地方的详细理由。
...
到目前为止见过**四次**。前两次都没抓到现场：第一次的证据只有那行错误，
...
### 2026-08-23：第四次，而现场又丢了

全量跑出 244/2，重跑就绿。**而我当时用的是 `python3 scripts/gotest.py ./... | tail -2`**
——那个脚本存在的全部理由就是留下现场，而我用管道把它截掉了。第六次犯同一个错。

处置不是「记得别截断」：`gotest.py` 现在把失败现场**写进临时文件**，
并把指向它的那一行印在**最末尾**——`tail` 保得住的地方。
**顺着那个习惯设计，而不是要求自己改掉它。**

（那两条是不是同一个 EOF flake，不知道——现场没了。这一条本身就是证据：
一次没留下现场的失败，事后再多的推测也补不回来。）
