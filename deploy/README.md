...
```sh
# 在节点上
sudo bash edge-node.sh update --agent-bin /path/to/new/edge-agent
```

**只换那一个二进制**，不碰配置、不碰状态目录、不碰 Caddy，也不需要 Token
（这台机器已经有隧道证书了）。换完等隧道连上；**30 秒内没连上就自动回滚** ——
一台连不上主控的节点收不到任何配置，包括「把它换回去」那条。

旧的留在 `/usr/local/bin/edge-agent.prev`，确认没问题之后可以删。

### 什么时候**必须**更新

| 功能 | 靠什么 | 旧 agent 能用吗 |
|---|---|---|
| IP 白名单 / 黑名单 | Caddy 原生匹配器 | **能** |
| 请求特征拦截 | 同上 | **能** |
| 限流 / CC 防护 | **Agent 的校验端点** | ✗ 必须更新 |
| 地域封禁 | **Agent 的校验端点 + GeoIP 库** | ✗ 必须更新 |
| 服务密钥、JWT | Agent 的校验端点（一直都有） | 能 |

前两种是主控渲染进 Caddy 配置的，旧 agent 只是把配置贴上去，**不需要认识里面的东西**。
后两种要 agent 自己会判。

> ### 装了新规则而没更新 agent 会怎样
>
> **那个域名的第一个请求就是 403。** 不是降级，是整站对所有人关闭 ——
> 而配置看起来完全正常。（实测过。）
>
> 校验端点是 fail-closed 的（[ADR-0003](../docs/adr/0003-edge-auth-via-agent-forward-auth.md)），
> 那对「这个请求没有凭据」是对的；而「这个节点不认识这条规则」是另一回事。
>
> **主控现在会拦住这种下发**：节点在握手时报出它认得哪些规则类型，
> 主控发现有节点认不出就整体拒绝，并点名是哪几台。
> 所以正常情况下你撞不到上面那一段 —— 你会先看到一句「先升级它们」。
>
