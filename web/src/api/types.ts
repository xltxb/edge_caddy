...
export interface NodeWire {
  ...
  /**
   * **过去一小时这台节点上被拦下了多少请求**（契约 §4）——
   * 限流的 429、以及处置方式为 403 / 404 的那些。
   *
   * `null` = **还不知道**：节点刚接入、或 Agent 刚重启，没有可比的两次心跳。
   *
   * ## 这是同一个 NodeWire 上第三个 `null`，而三个的处置各不相同
   *
   * | 字段 | `null` 的意思 | 界面 |
   * |---|---|---|
   * | `reconnects_1h` | 数不出来 | **必须说出来** |
   * | `geo_db_ok` | 不适用（主控没库） | **必须闭嘴** |
   * | `blocked_1h` | 还不知道 | **必须说出来** |
   *
   * 这一条跟 `reconnects_1h` 同档，理由一模一样：**它要回答的是
   * 「此刻在不在被打」，而 `0` 正是「没被打」的样子** —— 拿 0 当兜底，
   * 等于让它在自己失效的那一刻伪装成它最想否定的那个状态。
   *
   * 而 `geo_db_ok` 恰好相反。**照着任何一条的模式写另一条都会错**，
   * 而它们在同一个 interface 里隔着几十行。
   *
   * ## 三件界面上必须说的事
   *
   * **一、`abort` 那一档数不到。** 处置方式是「静默断连」时，被拦的请求
   * **不产生任何响应** —— Caddy 按状态码的计数里没有它（后端实测确认）。
   * 所以**这个数是 0 而规则确实在拦**是可能的，界面不能把 0 读成「没人来打」。
   *
   * **二、主控重启后从 0 重新攒**（窗口在内存里，与 `cpu_series` 同一条路）。
   * 不说清的话，一个刚重启完的主控看起来像刚过了一段太平日子。
   *
   * **三、每节点各算各的**，跟限流额度一样。
   */
