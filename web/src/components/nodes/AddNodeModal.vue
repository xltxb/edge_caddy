...
async function copy(which: "install" | "verify"): Promise<void> {
  ...
  const text =
    which === "install" ? issued.value.install_cmd : issued.value.verify_cmd;
  try {
    ...
    <div class="modal" role="dialog" aria-modal="true" aria-label="添加节点">
      ...
      <template v-if="!issued">
        ...
        <form class="form" @submit.prevent="submit">
          <label
            ><span>节点 ID</span
            ><input v-model="form.node_id" required placeholder="node-sg-01"
          /></label>
          <label
            ><span>城市</span
            ><input v-model="form.city" required placeholder="新加坡"
          /></label>
          <label
            ><span>服务商</span
            ><input v-model="form.vendor" required placeholder="V.PS"
          /></label>
          <label
            ><span>中转线路</span
            ><input v-model="form.line" required placeholder="CMIN2"
          /></label>
          <label
            ><span>公网 IP</span
            ><input v-model="form.public_ip" required placeholder="203.0.113.9"
          /></label>
      ...
      <template v-else>
        ...
        <div class="cmd-foot">
          <span
            >装完再跑这条 —— 它会真的去查 Agent 接没接上、Caddy Admin
            有没有监听在不该监听的地方</span
          >
        ...
        <ul class="checklist">
          ...
          <li>
            目标机器可以外连主控的 9000 端口（Agent 主动外连，无需入站放行）
          </li>
          <li>
            命令里的 <code>--ca-pin</code> 是主控的 CA
            指纹，<b>不要改也不要省</b>： 少了它，首连就是纯
            TOFU，中间人可以在那一刻把 Token 骗走。
          </li>
        </ul>
        <div class="actions">
