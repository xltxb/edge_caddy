...
<template>
  ...
  <div class="wrap">
    <form class="card" @submit.prevent="submit">
      ...
      <!--
        **这里说的是意图，不是事实。**
        ADR-0013 写着「『只绑内网』是**部署形态**而不是代码里的检查。代码只负责
        绑定地址可配，绑错地方（比如 0.0.0.0）不会有任何东西报警」。
        原先这句写的是「控制台只在内网可达」——一台被配成 0.0.0.0 的主控上，
        它会对着公网访问者宣称自己不可达（issue #78）。
      -->
      <p class="foot">控制台应当只部署在内网；登录成功与失败都会写入审计。</p>
    </form>
  </div>
</template>
