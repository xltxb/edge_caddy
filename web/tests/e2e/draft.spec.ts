...
test.describe('草稿', () => {
  ...
  test('改一个字段再改回去，待下发数回到原值', async ({ page }) => {
    ...
    await expect(pendingBadge(page)).toHaveText('3 项') // 该字段本来就在草稿里，仍是 1 处

    // 改回 live 值：这个字段应当从草稿里被删掉（契约 §6.4）
    await bodyMax.fill('5MB')
    await expect(pendingBadge(page)).toHaveText('2 项')
  ...
  test('放弃改动清掉该资源的全部草稿', async ({ page }) => {
    ...
    await expect(pendingBadge(page)).toHaveText('1 项')
    await page.reload()
    await expect(pendingBadge(page)).toHaveText('1 项')
  })
})
