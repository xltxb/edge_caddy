...
await check(
  ...
  async () => {
    ...
    /*
     * 钉的是：**不能出现一个格式正确而意思是假的值**。
     * `at` 要么是 null（从没同步过），要么是一个能解析的真实时刻。
     * 零值时间会被渲染成一个像模像样的 `00:00:00`，读起来像「凌晨同步过一次」，
     * 而空白会让人去查、一个像样的时间不会。
     *
     * （历史：第一版写的是「ok 为 false 时 at 必须是 null」，那是写窄了 ——
     * ok=false 覆盖「从没试过」和「试过但失败了」两种，后者的 at 是真实时刻，
     * 而且正是要给人看的那个「上次是什么时候没成功的」。）
     */
...
await check(
  ...
  async () => {
    ...
    /*
     * 这是一条**关于世界**的声明：主控给的默认值里 rate_limit 必须是 false。
     * true 是个下发一定会被拒的值（官方 Caddy 没有限流模块）。
     *
     * （历史：它原先是前端单测里的「默认不开限流」，而那测的是我自己夹具里写的
     * false —— 后端把默认改回 true 的话，那条照样绿。所以它归这里，不归单测。）
     */
    must(
