 ...
 *   drwx------  ./          ← 顶层目录 0700
 *
 * 灰度现场解出来是 `drwx------ root root`，主控跑在 `User=edge` 下，
 * 连目录都进不去。最刺眼的一幕：root 跑 `ls` 看到文件都在，而主控页面说
 * 「静态文件不在」——**两句话直接矛盾，因为看的人是 root，主控不是。**
 *
 * **中间那一步我复现不了，所以不写成断言。** 我这台机器上量过：
 *
 *   包里顶层 drwx------ ./
 *   带 --strip-components=1   解包后目标目录 drwxr-xr-x
 *   不带                      解包后目标目录 drwxr-xr-x
 *
 * 两种都没改目标目录的模式（macOS 的 BSD tar、非 root）。所以「归档里的
 * `./` 会盖到目标目录上」这条依赖 **Linux + GNU tar + root** 那个组合 ——
 * 现场的结果是真的，而机制我这边没有能证实它的环境。
 *
 * 这不影响该怎么做：**顶层 755 在哪种 tar 下都是对的**，而 0700 在至少一种
 * 组合下会出事。但它影响别的：修完这一处之后灰度仍然读不到文件，
 * 而如果我把这条当成已证实的全部原因，那条线就会被过早划掉。
 *
 * ## 成因不会消失
 *
