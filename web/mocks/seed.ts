...
const node = (
  ...
  reconnects_1h: 0,
  /*
   * **默认 `true`：库跟主控那份一致，界面上什么都不显示。**
   *
   * 三档都要在 dev 里走得到，而它们不是同一个维度上的三个值：
   *
   * - `true` / `false` 是**每台节点各自的**状态 —— 下面 node-jp-01 override 成
   *   `false`，那是「库没到或旧了，它上面的地域规则不生效」那一支。
   * - `null` 是**主控自己没有库**，那对所有节点是同一件事。所以它不该
   *   一台一台地给：造那一档要把整份 seed 的这个字段全设成 null，
   *   而不是让某一台单独为 null —— 后者在真主控上不可能出现。
   */
  geo_db_ok: true,
  /*
   * 默认 0（这一小时没被拦下任何请求）。**要看那个 UI 得 override** ——
   * 下面 node-kr-01 给了一个非零值，node-us-01 给 null（刚重启，还没攒够）。
   *
   * 三档都要在 dev 里走得到，而它们说的是不同的事：
   *   数字   拦下了这么多
   *   0     没有被拦下的 —— 但「静默断连」那一档数不进来
   *   null  还不知道（刚接入 / 主控刚重启）
   */
  blocked_1h: 0,
  /*
...
export const audit: AuditWire[] = [
  ...
  // 外部平台推证书进来（契约 §9 的 PUT /certs/:domain，走 cert-bot）。
  // 这里原先是「续期证书」——ADR-0015 之后没有续期这件事，而 mock 里躺着
  // 一条永远不可能真实发生的记录（issue #71）。
  au(2_649, 'cert-bot', '导入证书', 'cdn.example.com', '127.0.0.1', 'ok', '有效期至 2026-11-19'),
