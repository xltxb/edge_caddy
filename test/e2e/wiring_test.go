...
func TestMasterWiringLeavesNoComponentUnplugged(t *testing.T) {
	...
	cases := []struct {
		...
		{http.MethodGet, "/api/v1/certs", ""},
		{http.MethodPost, "/api/v1/certs/renew-all", ""},
		{http.MethodGet, "/api/v1/dns/provider", ""},
		{http.MethodPut, "/api/v1/dns/provider", `{"kind":"dnspod"}`},
		{http.MethodGet, "/api/v1/dns/schedule/cdn.example.com", ""},
		{http.MethodPut, "/api/v1/dns/schedule/cdn.example.com", `{"weights":[]}`},
