...
func main() {
	...
	// 清掉三张只写不清的表里已经没有价值的行（issue #34）。
	//
	// 一小时一次：这件事不急，而每次都是三条 DELETE。不接上这一行的话，
	// Prune 就是又一个「机制建好了，没接到会跑它的那个循环上」——
	// 这个仓库数到第八次的形状。
	go st.RunPrune(ctx, time.Hour, log)
