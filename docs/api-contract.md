...
- WS 复用同一个 Cookie，握手时校验；未登录直接 `401`，不升级。
  **连上之后还会复核**：主控在心跳周期上再看一次会话，发现没了就发一个带
  「会话已失效」的 `1000` 关闭帧（§2「服务端主动关闭」）。只在握手时校验的话，
  一条已经建起来的连接会在会话过期后继续推数据——而那些数据的读者早就无权看了。
...
**两边都挡**：控制台在 `web/src/ids.ts`，后端在 `model.ValidResourceID`。

后端那道不是多余的——§0.6 明说直改路（ops-bot、批量脚本）是给脚本的入口，
它绕过控制台。两个入口会创建新 id，都拒 `1001`：

| 端点 | 拒的是 |
|---|---|
| `POST /nodes/token` | `node_id`，排在绑定之后、查下线状态之前 |
| `PUT /rules/:id` | 路径上的 `id`（PUT 是 upsert，这条路会创建规则） |

> **是 `1001` 不是 `1002`。** §0.3 里 `1002` 要带结构化 `errors`、`data` 不为
> null，那是**字段级**校验的形状；而 id 不在请求体里（一个在路径上，一个是
> 单独一个字段），指不出「哪个字段错了」的那张表。

> `PUT /rules/:id` 校验 id，意味着库里一条历史遗留的坏 id 会变成只读。
> 那是对的：它本来就不该存在，而 `DELETE` 不校验，人仍然删得掉它。

> 两处的报错文案是同一句（`model.ResourceIDHint` 与 `RESOURCE_ID_HINT`）——
...
> |---|---|---|
> | `true` | 已推到服务商（**带上实际写入的记录名**） | 什么都不用做 |
> | `false` | 「当前没有可用的 DNS 服务商配置，解析未变动」 | 配置是空的（刚清空、或从来没配过）|
> | `false` | 「当前解析轮换里没有任何节点，解析未变动」 | **去把节点的解析开回来——凭证和网络都没问题** |
> | `false` | 「没能推上去」+ 这家服务商表达不了的理由 | 换一种安排，或换一家服务商 |
> | `false` | 「同步到服务商失败」+ 服务商回的原话 | 看那句原话：查网络、查凭证 |
> | `false` | 「本机未启用 DNS 编排，解析未变动」 | 这台主控没装编排器，是部署问题 |
>
> **中间那两档必须分开。** 「轮换是空的」是一次计划内的全网维护，说成
> 「同步失败」会把人送去查凭证、查网络、翻服务商的状态页，而那边什么毛病
> 也没有。后端为此给了它单独的错误类型（`ErrNothingInRotation`）。
>
> **只有「同步到服务商失败」那一档进 error 日志。** 其余几档都是有人做过的
> 某个决定，把它们记成 error 会让日志里的 error 变得不值得看——而那正是
> 真出事那次没人注意到的原因。
