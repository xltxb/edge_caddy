...
数据来自 **Caddy 自己的按状态码计数**，不是 Agent 另加的计数器——
另加的那个只数得到走校验端点的那几种（限流、地域），而 IP 黑名单与请求特征
是 Caddy 原生匹配器拦的，**它看不见**。

> 口径只取 `handler="headers"` 那一行：同一个请求会在链上每个 handler 各记一次，
> 求和会重复计数。而响应头处理挂在最前面，每个请求必经。
